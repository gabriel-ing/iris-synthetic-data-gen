from __future__ import annotations

from pandas.testing import assert_frame_equal

from conftest import build_dataset, load_small_config
from DataGen.validate import validate_all


def test_generation_is_deterministic_for_same_seed(tmp_path):
    cfg_a = load_small_config(tmp_path / "a", seed=123)
    a = build_dataset(cfg_a)

    cfg_b = load_small_config(tmp_path / "b", seed=123)
    b = build_dataset(cfg_b)

    for key in a:
        assert_frame_equal(a[key], b[key], check_like=False)


def test_generation_passes_core_validation(tmp_path):
    cfg = load_small_config(tmp_path)
    ds = build_dataset(cfg)

    result = validate_all(
        cfg,
        ds["calendar"],
        ds["roles"],
        ds["users"],
        ds["user_store_access"],
        ds["stores"],
        ds["products"],
        ds["supplier_product"],
        ds["promotions"],
        ds["purchase_orders"],
        ds["stock_transfers"],
        ds["sales_transactions"],
        ds["inventory_snapshot"],
    )

    assert result.ok is True
    assert len(result.checks) >= 28
    assert len(ds["stores"]) == cfg["resolved_counts"]["stores"]
    assert len(ds["products"]) == cfg["resolved_counts"]["products"]
    assert len(ds["sales_transactions"]) == cfg["resolved_counts"]["sales_transactions"]
    assert len(ds["inventory_snapshot"]) == cfg["resolved_counts"]["inventory_snapshots"]