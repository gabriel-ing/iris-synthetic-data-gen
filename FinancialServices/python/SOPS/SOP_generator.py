...
prompt = """You are writing an internal Standard Operating Procedure (SOP) for a card payments operations team at a fictional financial services company.

Write the SOP as natural, human-authored operational guidance. It must read like a real internal document, not like a template or a spec. Avoid heavy structure (no rigid sections like “1. Purpose, 2. Scope, 3. Definitions…”). Instead use short paragraphs, occasional bullet lists, and practical step-by-step language.

Hard requirements (follow all):
- The SOP must be useful to an agent or analyst who will query a database and may take limited actions.
- Incorporate the exact allowed values for relevant enums when they matter (include them in prose, not as a table).
- Mention the specific database tables and key fields that are relevant, but weave them into sentences (do not list columns exhaustively).
- Provide decision guidance using plain language “if / then” rules (no formal decision tables).
- Include at least:
  - 2 short realistic scenarios (3–6 sentences each) showing how the SOP is applied
  - 1 “common pitfalls” paragraph describing likely mistakes
  - 1 “what to write in the audit note” paragraph with an example audit note (free text)
  - 6–12 “retrieval hooks”: synonyms and alternate phrasing that people might use when searching (e.g., “chargeback” vs “dispute”, “tap” vs “contactless”), embedded naturally in the text
- Keep the document cohesive and narrative. Do not produce JSON/YAML. Do not output analysis or commentary. Output only the SOP text.

Actions and governance:
- Assume the system can read and write to IRIS SQL, but only certain actions are allowed depending on the SOP.
- If the SOP includes actions that modify data, clearly state:
  - What actions are allowed (e.g., “set Cards.Status to BLOCKED”)
  - What actions are not allowed without escalation (e.g., “do not set CLOSED automatically unless…”)
  - What evidence must be present before executing an action (cite fields and values)

Data model context (use these terms as written):
Tables:
- ModestExpectationsCaptial.Customers(CustomerId, CreatedAt, Status, Segment, RiskScore, Region, SegmentTxnMultiplier, SegmentAmountMultiplier, SegmentEcomMultiplier, SegmentDeclineMultiplier, SegmentDisputeMultiplier)
- ModestExpectationsCaptial.Cards(CardId, CustomerId, CardType, Status, OpenedAt, ClosedAt, CardToken, CreditLimit)
- ModestExpectationsCaptial.Merchants(MerchantId, MerchantName, Category, RiskTier, PopularityWeight, Country)
- ModestExpectationsCaptial.Transactions(TransactionId, CardId, MerchantId, MerchantName, AuthAt, PostedAt, Amount, Currency, Channel, EntryMode, CardPresent, Status, DeclineReason, IsFraud)
- ModestExpectationsCaptial.Disputes(DisputeId, TransactionId, OpenedAt, ResolvedAt, ReasonCode, State, Outcome, DisputedAmount)

Allowed values you must respect:
- Cards.Status: ACTIVE, BLOCKED, CLOSED
- Merchants.RiskTier: LOW, MED, HIGH
- Merchants.Category: GROCERY, DIGITAL, RETAIL, FUEL, TRAVEL, DINING
- Transactions.Channel: ECOM, POS
...
Now write the SOP described below.
...
Key fields to reference (use as anchors, but don’t dump lists):
{key_fields}

Allowed actions for this SOP:
{allowed_actions}

Golden questions this SOP should answer:
{golden_questions}

Tone:
Concise, operational, and practical. Avoid legalistic compliance wording. Avoid marketing language.
Length:
800–1400 words."""

openai_client = OpenAI()
def generate_sop(sop_details: dict) -> str:

    sop_prompt = prompt.format_map(sop_details)

    
    response = openai_client.chat.completions.create(
        model="gpt-5",
        messages=[{"role": "system", "content": "You are a helpful assistant that writes SOPs."},
                  {"role": "user", "content": sop_prompt}],
    )

    with open(f"./out/{sop_details['sop_id']}.txt", "w", encoding="utf-8") as f:
        f.write(response.choices[0].message.content)


if __name__ == "__main__":
    sop_name = "Transaction Monitoring and Fraud Detection"
    for sop in sops[1:]:
        generate_sop(sop)
    # print(f"Generated SOP for '{sop_name}':\n\n{sop}")